...
analysis.dir = "./output/byExperiment"
report.dir = file.path(analysis.dir, "Oct1_pairedCompsByExperiment")
...
my.pheno <- read.csv(file = './input/my.pheno.per.exp.csv',header = T,sep = ',',quote = '',na.strings = 'NA')
...
my.range = c(1:nrow(my.pheno))
my.pheno = my.pheno[my.range,]
betas = betas[,my.range]
p.vals = p.vals[,my.range]
my.diff.comps <- colnames(my.pheno)[5:ncol(my.pheno)]
...
rnb.options("qc.boxplots"=TRUE, "qc.barplots"=TRUE, "qc.negative.boxplot"=FALSE,
            ...
            "qc.sample.batch.size"=50, "qc.coverage.plots"=T,
            "qc.coverage.threshold.plot"=1:10,"qc.coverage.histograms"=T,
