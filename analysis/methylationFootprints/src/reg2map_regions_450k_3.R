...
regions_dyad_E122 <- read.delim("../../data/raw/reg2map/regions_dyadic_E122.bed", header=FALSE)
...
grDyad <- GRanges(seqnames = regions_dyad_E122$V1,
                 ranges = IRanges(start = regions_dyad_E122$V2,end = regions_dyad_E122$V3))
...
#These probes represent the intersection of regulatory regions with 450k array probes
enhProbes <- subsetByOverlaps(gr450Kprobes,grEnh)
promProbes <- subsetByOverlaps(gr450Kprobes,grProm)
dyadProbes <- subsetByOverlaps(gr450Kprobes,grDyad)

regTypeProbes <- promProbes
...
grTF.ChIPseq = GRanges(seqnames = my.current.TF$V1,
                        ranges = IRanges(start = (my.current.TF$V2-2000),
                                         end =(my.current.TF$V3+2000))
                       ) 

##############################################################
#################################################################
TF.REG <- subsetByOverlaps(regTypeProbes,grTF.ChIPseq)

if(all(c(length(TF.REG@elementMetadata@listData$mcols.EBM1.AVG_Beta),
       length(TF.REG@elementMetadata@listData$mcols.EGM1.AVG_Beta),
       length(TF.REG@elementMetadata@listData$mcols.EVpellet1.AVG_Beta),
        length(TF.REG@elementMetadata@listData$mcols.EVsuper1.AVG_Beta),
        length(TF.REG@elementMetadata@listData$mcols.UCMpellet1.AVG_Beta),
        length(TF.REG@elementMetadata@listData$mcols.UCMsuper1.AVG_Beta)) != rep(0,6))){

plot(ecdf(TF.REG@elementMetadata@listData$mcols.EBM1.AVG_Beta),col = 'red1', cex = .2)
lines(ecdf(TF.REG@elementMetadata@listData$mcols.EGM1.AVG_Beta),col = 'orange', cex = .2)
lines(ecdf(TF.REG@elementMetadata@listData$mcols.EVpellet1.AVG_Beta),col = 'green1', cex = .2)
lines(ecdf(TF.REG@elementMetadata@listData$mcols.EVsuper1.AVG_Beta),col = 'purple', cex = .2)
lines(ecdf(TF.REG@elementMetadata@listData$mcols.UCMpellet1.AVG_Beta),col = 'black', cex = .2)
lines(ecdf(TF.REG@elementMetadata@listData$mcols.UCMsuper1.AVG_Beta),col = 'pink', cex = .2)
}
