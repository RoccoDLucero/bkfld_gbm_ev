...
#This code differs from the first version in that the cumputation
#of the 2x2 contingency table accounts for the split of the intial
#DMR counts into those that had hits and those that did not
#The intersect beds input here should be such that each probe is tested as a hit or miss
#vs each TF tested
...
my.comp.TF.enr = function(inputDir = "./", nameElems = c('*count*','*.bed'),
    ...
    for(row in 1:nrow(my.intersects)){
        ...
        enr.tmp = (tab[1,1]/(tab[2,1]-tab[1,1]))/(tab[1,2]/(tab[2,2]-tab[1,2]))
